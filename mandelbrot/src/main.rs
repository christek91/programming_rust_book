...
/// Try to detrmin ie 'c' is in the Mandelbrot set, using at most `limit` iterations to decide.
///
/// If `c` is not a member, return 'Some(i)', where 'l' is the number of
/// iterations it took for 'c' to leave the circle of radisu 2 centered on the
...
/// arguments specificy points on the complex plane corresponding to the upper-left
